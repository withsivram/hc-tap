...
def check_makefile_targets():
    ok = {"bootstrap": False, "extract-local": False, "eval": False, "api-stub": False, "dash": False, "clean": False, "help": False}
    p = ROOT / "Makefile"
    txt = read_text(p)
    for t in ok:
...
def check_ci_workflow():
    ...
    res = {
        ...
        "workflow_name": (re.search(r"^name:\s*(.+)$", txt, flags=re.M) or [None, ""])[1].strip(),
    }
    return res

def basic_schema_validation():
    out = {"note_schema_present": exists("contracts/note.schema.json"), "entity_schema_present": exists("contracts/entity.schema.json"), "validated": False, "errors": []}
    try:
    ...
    try:
        ...
        for path in glob.glob(str(ROOT / "fixtures/entities/*.jsonl")):
            for i, line in enumerate(read_text(Path(path)).splitlines(), 1):
                ...
                if err:
                    out["errors"].append(f"entities schema error in {path}:{i}: {err.message}")
                note_text = notes.get(obj.get("note_id"))
                if note_text is not None:
                    b, e = obj.get("begin"), obj.get("end")
                    if not (isinstance(b,int) and isinstance(e,int) and 0 <= b < e <= len(note_text)):
                        out["errors"].append(f"span out of bounds in {path}:{i} (begin={b}, end={e}, len={len(note_text)})")
        if not out["errors"]:
            out["validated"] = True
    except Exception as e:
...
def main():
    ...
    print(f" - triggers pull_request: {'OK' if ci['has_pull_request_trigger'] else 'MISSING'}")
    print(f" - python 3.12: {'OK' if ci['uses_python_312'] else 'CHECK'}")
    print(f" - pip cache: {'OK' if ci['has_pip_cache'] else 'MISSING'}")
    print(f" - installs requirements: {'OK' if ci['installs_requirements'] else 'MISSING'}")
