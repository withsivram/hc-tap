...
with open("contracts/entity.schema.json","r",encoding="utf-8") as f: ENTITY_SCHEMA=json.load(f)
with open("contracts/note.schema.json","r",encoding="utf-8") as f:   NOTE_SCHEMA=json.load(f)
entity_validator=jsonschema.Draft202012Validator(ENTITY_SCHEMA)
note_validator=jsonschema.Draft202012Validator(NOTE_SCHEMA)

# Validate notes and keep text for span checks
notes={}
for path in glob.glob("fixtures/notes/*.json"):
  ...
  if err:
    ...
    print(f"[notes] Schema error in {path} at {loc}: {err.message}"); sys.exit(1)
...
for path in glob.glob("fixtures/entities/*.jsonl"):
  with open(path,"r",encoding="utf-8") as f:
    for lineno,line in enumerate(f,1):
      ...
      if err:
        loc="/".join([str(p) for p in err.path]) or "(root)"
        print(f"[entities] Schema error in {path}:{lineno} at {loc}: {err.message}"); sys.exit(1)
