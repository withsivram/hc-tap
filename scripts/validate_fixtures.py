...
with open("contracts/entity.schema.json", "r", encoding="utf-8") as f:
    ENTITY_SCHEMA = json.load(f)
with open("contracts/note.schema.json", "r", encoding="utf-8") as f:
    ...
    with open(path, "r", encoding="utf-8") as f:
        for lineno, line in enumerate(f, 1):
            ...
            if err:
                loc = "/".join([str(p) for p in err.path]) or "(root)"
                print(
                    f"[entities] Schema error in {path}:{lineno} at {loc}: {err.message}"
                )
                sys.exit(1)
            ...
            if nt is not None:
                ...
                if not (
                    isinstance(b, int) and isinstance(e, int) and 0 <= b < e <= len(nt)
                ):
                    print(
                        f"[entities] span out of bounds in {path}:{lineno} (begin={b}, end={e}, len={len(nt)})"
                    )
                    sys.exit(1)
            entity_count += 1
